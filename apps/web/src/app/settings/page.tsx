...
const TENANT_ID = '00000000-0000-0000-0000-000000000000'; // Placeholder for auth
...
export default function SettingsPage() {
  ...
  const { data: settings = [], isLoading } = useQuery({
    queryKey: ['settings', TENANT_ID, 'risk_rules'],
    queryFn: () => fetchApi<SettingResponse[]>(`/settings/${TENANT_ID}?category=risk_rules`),
  });
  ...
  const saveDraftMutation = useMutation({
    mutationFn: (payload: RiskRulesPayload) => {
      if (currentSetting) {
        return fetchApi(`/settings/${TENANT_ID}/${currentSetting.id}`, {
          method: 'PUT',
          body: JSON.stringify({ payload }),
        });
      } else {
        return fetchApi(`/settings/${TENANT_ID}`, {
          method: 'POST',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', TENANT_ID, 'risk_rules'] });
    }
  });

  const publishMutation = useMutation({
    mutationFn: (settingId: string) => fetchApi(`/settings/${TENANT_ID}/${settingId}/publish`, { method: 'POST' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settings', TENANT_ID, 'risk_rules'] });
    }
  });
